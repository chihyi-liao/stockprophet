...
class CrawlerTask(threading.Thread):
    ...
    def build_stock_table(self) -> bool:
        ...
        if len(metadata) == 1:
            db_mgr.stock_metadata.update_api(
                ...
                update_data={'tse_stock_info_update_date': dt})
        return True
    ...
    def run(self):
        ...
        if len(metadata) == 1:
            stock_info_update_date = metadata[0]['tse_stock_info_update_date']
            weekly_history_update_date = metadata[0]['tse_weekly_history_update_date']
            monthly_history_update_date = metadata[0]['tse_monthly_history_update_date']
