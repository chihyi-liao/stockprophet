...
from stockprophet.db.model.stock_monthly_history import StockMonthlyHistory
...
def create_api(s: Session, data_list: list = None) -> bool:
    ...
    if data_list:
        s.execute(StockMonthlyHistory.__table__.insert(), data_list)
        s.commit()
        result = True
...
def read_api(s: Session, code: str,
    ...
    """依據股票代號查詢每月股市歷史資料"""
    result = []
    h_obj = StockMonthlyHistory.__table__.c
    ...
    stmt = select([
        ...
        StockMonthlyHistory.__table__.join(
            StockMonthlyDate.__table__, d_obj.id == h_obj.stock_date_id
        ).join(
...
def update_api(s: Session, oid: int, update_data: dict = None) -> bool:
    ...
    if update_data:
        obj = StockMonthlyHistory.__table__.c
        stmt = update(StockMonthlyHistory.__table__).where(obj.id == oid).values(update_data)
        s.execute(stmt)
        s.commit()
        result = True
...
def delete_api(s: Session, oid: int) -> bool:
    """依據id刪除每月股市歷史資料"""
    obj = StockMonthlyHistory.__table__.c
    stmt = delete(StockMonthlyHistory.__table__).where(obj.id == oid)
    s.execute(stmt)
    s.commit()
    return True
