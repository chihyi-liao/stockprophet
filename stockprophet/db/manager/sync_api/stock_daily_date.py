...
def create_api(s: Session, data_list: list = None) -> bool:
    ...
    query = insert(stock_daily_date_table).values(data_list)
    try:
        s.execute(query)
        s.commit()
        result = True
...
def read_api(s: Session, date: datetime.date) -> list:
    ...
    result = []
    query = select(
        [stock_daily_date_table.c.id, stock_daily_date_table.c.date]
    ).where(stock_daily_date_table.c.date == date).limit(1)
    try:
...
def readall_api(s: Session, type_s: str) -> list:
    ...
    ).order_by(asc(stock_daily_date_table.c.id)).where(stock_table.c.stock_type_id == subquery).distinct()
...
def update_api(s: Session, oid: int, update_data: dict = None) -> bool:
    ...
    query = update(stock_daily_date_table).where(stock_daily_date_table.c.id == oid).values(update_data)
    try:
        s.execute(query)
        s.commit()
        result = True
...
def delete_api(s: Session, oid: int) -> bool:
    """依據id刪除股市日期"""
    result = False
    query = delete(stock_daily_date_table).where(stock_daily_date_table.c.id == oid)
    try:
        s.execute(query)
        s.commit()
        result = True
