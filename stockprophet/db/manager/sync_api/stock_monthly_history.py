...
def create_api(s: Session, data_list: list = None) -> bool:
    ...
    query = insert(stock_monthly_history_table).values(data_list)
    try:
        s.execute(query)
        s.commit()
        result = True
...
def read_api(s: Session, code: str,
    ...
    query = select([
        stock_monthly_history_table.c.id, stock_monthly_date_table.c.date,
        stock_table.c.code, stock_table.c.name,
        stock_monthly_history_table.c.op, stock_monthly_history_table.c.hi,
        stock_monthly_history_table.c.lo, stock_monthly_history_table.c.co,
        stock_monthly_history_table.c.vol, stock_monthly_history_table.c.val, stock_monthly_history_table.c.ch]
    ).select_from(
        stock_monthly_history_table.join(
            stock_monthly_date_table, stock_monthly_date_table.c.id == stock_monthly_history_table.c.stock_date_id
        ).join(
...
def readall_api(s: Session, type_s: str, stock_date: datetime.date = None) -> list:
    ...
    ).order_by(asc(stock_monthly_history_table.c.stock_id))
...
def update_api(s: Session, oid: int, update_data: dict = None) -> bool:
    ...
    query = update(stock_monthly_history_table).where(stock_monthly_history_table.c.id == oid).values(update_data)
    try:
        s.execute(query)
        s.commit()
        result = True
...
def delete_api(s: Session, oid: int) -> bool:
    """依據id刪除每月股市歷史資料"""
    result = False
    query = delete(stock_monthly_history_table).where(stock_monthly_history_table.c.id == oid)
    try:
        s.execute(query)
        s.commit()
        result = True
